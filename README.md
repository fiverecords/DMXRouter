...
- **~114,000 lines of production C++17** — zero compiler warnings with strict flags (`-Wall -Wextra -Wpedantic` / `/W4`)
...
- **Automatic Spotme port discovery** — the receiver listens on the full Spotme port range `56565-56820` (256 ports = every legal "PSN SpotMe ID" 0..255) by default. The operator doesn't have to know that Spotme's "PSN ID" parameter is a port offset from 56565, doesn't have to calculate which port a given chassis is on, and doesn't have to reconfigure DMXRouter when adding a new Spotme to the rig — every chassis just appears in the Servers panel with its derived ID shown next to the port. The PSN spec default `56565` is included in the range, so non-Spotme tracking sources (BlackTrax, Zactrack, Maestro, MA Stage Marker streams) work out of the box. Operators with trackers running on unusual ports outside the default range can extend the listened set with a comma-separated expression in the toolbar's Ports field (e.g. `56565-56820, 60000-60010`); the field accepts the same range syntax as the Universe Monitor's filter and caps at 1024 ports total
...
- **Settings persistence** — the multicast address, port set, and enable state are saved across sessions via QSettings. Defaults are the spec's `236.10.10.10` group with the full SpotMe port range `56565-56820` (which also includes the spec default `56565` for non-SpotMe trackers). Apply validates that the entered address is in the IPv4 multicast range (`224.0.0.0/4`) and rejects others with an inline status message; ports are validated as a comma-separated list of single ports and `low-high` ranges within `1-65535`. Corrupt or out-of-range saved values fall back to the default silently. A v1.10.0-or-earlier single-port setting is migrated to a one-port expression on first load so upgrading users see their previous configuration preserved without having to know the new syntax
...
- `DMXRouter-v1.10.1-linux-x86_64.zip` — standard PCs and servers
- `DMXRouter-v1.10.1-linux-arm64.zip` — Raspberry Pi 4/5, Orange Pi, and other ARM64 boards
