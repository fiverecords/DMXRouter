...
- **Universe merge engine** — 8 merge modes including HTP, LTP, Backup, X-Fade, Switch, and Custom per-channel policy. Per-engine failsafe behaviour (Hold Last State, Blackout, Full On, Playback Scene, or Stop Output entirely so a lower-priority sACN source on the same universe takes over) when every input is lost
...
- **PSN viewer** — passive PosiStageNet receiver and visualiser for tracking-server traffic (Spotme, BlackTrax, Zactrack, MA Stage Marker streams, etc.). Multi-server support, sortable table with server-coloured swatches, interactive 2D top-down map with mouse-wheel zoom, drag-pan, and Fit View. Click a row to highlight the corresponding tracker on the map. Receive-only — DMXRouter never emits PSN, so it's safe to drop onto a live tracking network as a passive observer
- **Manual test patterns** — per-engine "Test…" button (toolbar + right-click) opens a non-modal dialog that injects Full / Blackout / Ramp / Chase / Sinewave (synchronous) / Sinewave (rolling) / Single Channel patterns onto an engine's outputs while troubleshooting. Bypasses channel patch and fixture overrides — what hits the wire is exactly the pattern. Works on silent engines too (data loss / Stop failsafe / never-received-input — DMXRouter feeds the pattern at ~30 Hz on its own). Live pattern switching, Slow / Medium / Fast speed presets, configurable Chase group size (1–16, set to 3 for RGB pixel strips, 4 for RGBW) and rolling-Sinewave wavelength (3–512 channels), selectable duration with a 5-minute hard ceiling, multi-engine simultaneous testing, live "TEST 23s" countdown badge in the engine row, auto-stop on dialog close / engine edit / profile load. Inspired by sACNView's Transmit Window and ChamSys MagicQ's per-universe Test button
...
- **Sortable columns** — click the Server, ID, or Name header to reorder the table; click again to flip direction. The numeric position / speed / orientation columns are deliberately not sortable since they update at the receiver's frame rate (up to 250 Hz per spec) and would make rows shuffle continuously
...
- **Settings persistence** — the multicast address, port, and enable state are saved across sessions via QSettings. Defaults are the spec's `236.10.10.10:56565`; Apply validates that the entered address is in the IPv4 multicast range (`224.0.0.0/4`) and rejects others with an inline status message. Corrupt or out-of-range saved values fall back to the default silently
...
- `DMXRouter-v1.10.0-linux-x86_64.zip` — standard PCs and servers
- `DMXRouter-v1.10.0-linux-arm64.zip` — Raspberry Pi 4/5, Orange Pi, and other ARM64 boards
...
*DMXRouter v1.10.0 — Built for the stage.*
